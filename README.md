...
**[Land Use–ABM–DTA Practice Studio](https://asu-trans-ai-lab.github.io/TensorMobility/explainer/practice.html)** (grid size & tours as controls; emergent vehicle conflicts; retiming feedback; the **annual land-use iteration** — the long-term loop — with exact household conservation) ·
...
`teaching/` carries the T0–T7 ladder (mobility-tensor paper case →
FTT chain by hand → logit/UE → fluid queues → pool audit →
compression boundary → prices as synchronization → mixed autonomy +
engines). T0 is runnable today:
`python cases/run_mobility_tensor_paper.py`. See
[teaching/README.md](teaching/README.md).

## Papers & docs

- `research_papers/trb_mobility_tensor/` — TRB paper draft: *A Typed
  Multi-Axis Mobility Tensor for Integrated Activity-Tour Choice and
  Congested Network Assignment*. Every number regenerated by
  `python cases/run_mobility_tensor_paper.py` (teaching case T0 is the
  same artifact).
